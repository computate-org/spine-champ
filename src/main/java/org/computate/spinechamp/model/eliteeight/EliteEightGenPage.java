 ...
 * Generated: true
 **/
public class EliteEightGenPage extends EliteEightGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListEliteEight_(Wrap<SearchList<EliteEight>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListEliteEight_ != null)
      w.o(Optional.ofNullable(searchListEliteEight_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListEliteEight_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListEliteEight_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListEliteEight_.getStart().longValue();
    Long rows = searchListEliteEight_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListEliteEight_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListEliteEight_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    EliteEight.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(EliteEight.displayNameEliteEight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(EliteEight.classSimpleNameEliteEight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListEliteEight_.getRequest().getQuery()).filter(fq -> fq.startsWith(EliteEight.varIndexedEliteEight(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : EliteEight.varsFqForClass()) {
      String varIndexed = EliteEight.varIndexedEliteEight(var);
      String varStored = EliteEight.varStoredEliteEight(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(EliteEight.displayNameEliteEight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(EliteEight.classSimpleNameEliteEight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListEliteEight_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(EliteEight.varIndexedEliteEight(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = EliteEight.searchVarEliteEight(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    EliteEight.varsRangeForClass().forEach(var -> {
      String varIndexed = EliteEight.varIndexedEliteEight(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(EliteEight.displayNameEliteEight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(EliteEight.classSimpleNameEliteEight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListEliteEight_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(EliteEight.varIndexedEliteEight(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListEliteEight_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListEliteEight_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListEliteEight_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListEliteEight_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListEliteEight_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = EliteEight.searchVarEliteEight(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", EliteEight.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListEliteEight_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = EliteEight.searchVarEliteEight(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", EliteEight.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListEliteEight_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListEliteEight_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListEliteEight_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListEliteEight_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return EliteEight.searchVarEliteEight(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListEliteEight_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListEliteEight_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListEliteEight_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListEliteEight_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListEliteEight_.getDefaultSort()) {
      Optional.ofNullable(searchListEliteEight_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = EliteEight.searchVarEliteEight(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListEliteEight_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = EliteEight.searchVarEliteEight(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListEliteEight_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListEliteEight_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listEliteEight(JsonArray l) {
    Optional.ofNullable(searchListEliteEight_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListEliteEight_ == null ? 0 : searchListEliteEight_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<EliteEight> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("bracketId")).orElse(null) != null)
      w.o(searchListEliteEight_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("elite eight brackets");
    else if(searchListEliteEight_ == null || resultCount == 0)
      c.o("no elite eight bracket found");
    else
      c.o("elite eight brackets");
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all elite eight brackets");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/elite-eight");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/elite-eight");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A predicted tournament with 8 teams. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/elite-eight-999.png");
  }
