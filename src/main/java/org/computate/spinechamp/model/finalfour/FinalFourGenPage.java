 ...
 * Generated: true
 **/
public class FinalFourGenPage extends FinalFourGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListFinalFour_(Wrap<SearchList<FinalFour>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListFinalFour_ != null)
      w.o(Optional.ofNullable(searchListFinalFour_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListFinalFour_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListFinalFour_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListFinalFour_.getStart().longValue();
    Long rows = searchListFinalFour_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListFinalFour_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListFinalFour_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    FinalFour.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(FinalFour.displayNameFinalFour(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(FinalFour.classSimpleNameFinalFour(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListFinalFour_.getRequest().getQuery()).filter(fq -> fq.startsWith(FinalFour.varIndexedFinalFour(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : FinalFour.varsFqForClass()) {
      String varIndexed = FinalFour.varIndexedFinalFour(var);
      String varStored = FinalFour.varStoredFinalFour(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(FinalFour.displayNameFinalFour(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(FinalFour.classSimpleNameFinalFour(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListFinalFour_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(FinalFour.varIndexedFinalFour(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = FinalFour.searchVarFinalFour(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    FinalFour.varsRangeForClass().forEach(var -> {
      String varIndexed = FinalFour.varIndexedFinalFour(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(FinalFour.displayNameFinalFour(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(FinalFour.classSimpleNameFinalFour(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListFinalFour_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(FinalFour.varIndexedFinalFour(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListFinalFour_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListFinalFour_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListFinalFour_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListFinalFour_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListFinalFour_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = FinalFour.searchVarFinalFour(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", FinalFour.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListFinalFour_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = FinalFour.searchVarFinalFour(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", FinalFour.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListFinalFour_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListFinalFour_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListFinalFour_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListFinalFour_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return FinalFour.searchVarFinalFour(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListFinalFour_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFinalFour_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFinalFour_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListFinalFour_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListFinalFour_.getDefaultSort()) {
      Optional.ofNullable(searchListFinalFour_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = FinalFour.searchVarFinalFour(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListFinalFour_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = FinalFour.searchVarFinalFour(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListFinalFour_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListFinalFour_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listFinalFour(JsonArray l) {
    Optional.ofNullable(searchListFinalFour_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListFinalFour_ == null ? 0 : searchListFinalFour_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<FinalFour> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("bracketId")).orElse(null) != null)
      w.o(searchListFinalFour_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("final four brackets");
    else if(searchListFinalFour_ == null || resultCount == 0)
      c.o("no final four bracket found");
    else
      c.o("final four brackets");
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all final four brackets");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/final-four");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/final-four");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A predicted tournament with 4 teams. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/final-four-999.png");
  }

  @Override
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"{{ FONTAWESOME_STYLE }} fa-person-basketball\"></i>");
  }

  protected void _pageUriFinalFour(Wrap<String> c) {
      c.o("/en-us/search/final-four");
  }
}
